...
def get_model_metrics():
    # Simplified metrics showing only accuracy for each model
    metrics = {
        'Model': ['Logistic Regression', 'Random Forest', 'XGBoost', 'Decision Tree'],
        'Accuracy': [0.51, 0.86, 0.96, 0.65]
    }
...
def display_model_comparison_chart():
    ...
    bars = ax.bar(bar_positions, metrics_df['Accuracy'], width=0.6, color=['#1f77b4', '#2ca02c', '#d62728', '#9467bd'])
...
def main():
    ...
    with tab2:
        ...
        # Display model metrics table - simplified to show only accuracy
        metrics_df = get_model_metrics()
        ...
        with col1:
            st.info("**Logistic Regression (51%)**: A linear model for binary classification. Simple, interpretable, but misses complex patterns.")
            st.info("**Random Forest (86%)**: Ensemble of decision trees that reduces overfitting. Good balance of accuracy and interpretability.")
        
        with col2:
            st.info("**XGBoost (96%)**: Advanced gradient boosting implementation. Highest accuracy but less interpretable.")
            st.info("**Decision Tree (65%)**: Simple tree-based model with explicit decision rules. Highly interpretable but less accurate.")
